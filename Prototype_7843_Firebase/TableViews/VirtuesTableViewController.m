//
//  VirtuesTableViewController.m
//  Prototype_7843_Firebase
//
//  Created by Jeff on 7/8/21.
//

#import "VirtuesTableViewController.h"
#import "EditingVirtuesViewController.h"
#import "DataTableViewCell.h"

@interface VirtuesTableViewController (){
   
}

@end

@implementation VirtuesTableViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    //setup firestore
    self.firestore = [FIRFirestore firestore];
    //get the data from firestore
    FIRCollectionReference *virtueCollectionRef = [[self firestore] collectionWithPath:@"Virtues"];
    ...
    [virtueCollectionRef addSnapshotListener:^(FIRQuerySnapshot * _Nullable snapshot, NSError * _Nullable error) {
        if (error == nil){
            self->_virtueArray = [[NSMutableArray alloc] init];
            //loop all the data inside firebase
            for (FIRDocumentSnapshot *document in snapshot.documents) {
                //get the data from database and save in array
                Virtues *virtue = [[Virtues alloc] initWithDictionary:[document data]];
                virtue.autoId = document.documentID;
                //self=global variable
               [self->_virtueArray addObject:virtue];
           }
    ...
    //remove empty space under table view
...
//return the table cell at the index path "Virtues"
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    //check default swipe from left
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        //remove the deleted object from your data source.
        //If your data source is an NSMutableArray, do this
        Virtues *model = self.virtueArray[indexPath.row];
        [[[self.firestore collectionWithPath:@"Virtues"] documentWithPath:model.autoId]
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    //redirect to editin editingVirtuesVC and send model to display and edit model(list of virtues)
    Virtues *model = [self.virtueArray objectAtIndex:indexPath.row];
    EditingVirtuesViewController *destinationvcname = [self.storyboard instantiateViewControllerWithIdentifier:@"EditingVirtuesViewController"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.lblDays setText:[NSString stringWithFormat:@"%ld",(long)[components day]]];
    [cell.lblName setText:[model name]];
    [cell.lblDate setText:[NSString stringWithFormat:@"%@",[model startDate]]];
